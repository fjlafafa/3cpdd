...
if not args.no_progress_bar:
    ...
    loader_val = tqdm(loader_val)
    loader_test = tqdm(loader_test)

best_rmsd, best_epoch = 100, -1
for e in range(args.epochs):
    model.train()
    loss_meter = AverageMeter()
    losses_meters = [AverageMeter() for _ in range(5)]
    ...
    for i,batch in enumerate(loader_train):
        ...
        if out.loss.isnan().sum().item() == 0:
            ...
            for losses_meter, ploss in zip(losses_meters, out.losses):
                losses_meter.update(ploss.item())
            out.loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), args.clip_norm)
            optimizer.step()

        if (i + 1) % args.print_iter == 0:
            print(f'[Iter {i + 1}] Train Loss = {loss_meter.avg:.3f}, dloss = {losses_meters[0].avg:.3f}, iloss = {losses_meters[1].avg:.3f}, vloss = {losses_meters[2].avg:.3f}, rloss = {losses_meters[3].avg:.3f}, elapsed {time.time() - start} seconds. ')
            start = time.time()
            loss_meter.reset()
            for losses_meter in losses_meters:
                losses_meter.reset()
