...
class RefineDocker(ABModel):
    ...
    def __init__(self, args):
        ...
        if args.sparse_encoder:
            from bindgen.sparse_encoder import EGNNEncoder, HierEGNNEncoder
            # from bindgen.pretrain_encoder import PretrainEncoderWrapper as EGNNEncoder
        else:
            from bindgen.encoder import EGNNEncoder, HierEGNNEncoder
    ...
    def struct_loss(self, bind_X, tgt_X, true_V, true_R, true_D, inter_D, true_C):
        ...
        # we generate bind_X which represents the 3d coordinate of the antibody
        # vloss: mse loss of dihedral angles in antibody, 
        # rloss: atom distance within each aa in antibody, 
        # dloss: antibody aa distance as characterized by alpha carbon, 
        # closs: antibody and antigen atom distance, 
        # iloss: antibody and antigen aa distance,
        ...
        # print("rdist:", rdist, "trueR:", true_R)
        # exit(0)
    ...
    def forward(self, binder, target, surface):
        ...
        # Todo: what is tgt_surface and surface
        tgt_S = self.embedding(tgt_S)
        tgt_V = self.features._dihedrals(tgt_X)
        tgt_h, _ = self.target_mpn(tgt_X, tgt_V, tgt_S, tgt_A)
        ...
        if self.hierarchical:
            ...
            closs = torch.sum(closs[:,:,:4,:4]) / cmask_2D[:,:,:4,:4].sum()

        losses = [dloss, iloss, vloss, rloss]
        loss = sum(losses)
        
        return ReturnType(loss=loss, bind_X=bind_X.detach(), handle=(tgt_X, tgt_A), losses=losses)
